...
int cnt2 =0;
double thread_t2[6][1000]={0,};//0==cnt, 1==Euler, 2==Ang_vel, 3==thread_t 4==
...
//imu
    ...
    cout << "while 끝 thread is running =false 이전 " << endl;    
    // initializeKubot();
    //while까지만 실행되는 듯 밑에 있는 thread_is_running = false은 실행되지 않음
...
void save_file(){
    ...
    if(file.is_open()) {
		file <<"count"<<","<<"Euler"<<","<<"ang_vel"<<","<<"dt_imu"<<","<<"process_thread"<<","<<"dt_thread"<<"\n";
...
void initializeKubot() {
    ...
    q.R.init = VectorXd::Zero(6);//모든 다리 관절을 0도로 초기화 해준다.
...
void serial_task(void* arg)
{
    ...
    rt_task_set_periodic(NULL, TM_NOW, cycle_ns * 5);
    ...
    while (thread_is_running)
    {
        ...
        else if(t < C.zmp.previewControl.RefTotalTime - 4*C.zmp.previewControl.PreviewTime) {
             process();
             cout << "t =  "<< t << endl;
        }
	    else if(t < C.zmp.previewControl.RefTotalTime){
        ...
        C.walking.time.sec = t - C.zmp.previewControl.PreviewReadyTime - C.walking.step.current * C.walking.time.periodTime;//sec은 현재시간- 2초 - 현재 발걸음*주기
        //C.generateFootTraj();-------------> 이건 예전버전이라 주석
       ...
       //if (t < C.zmp.previewControl.RefTotalTime - 2*C.zmp.previewControl.PreviewTime)
        if (t < C.zmp.previewControl.RefTotalTime - 4*C.zmp.previewControl.PreviewTime) {
            if(C.walking.time.sec >= C.walking.time.SSP_start_time && C.walking.time.sec <= C.walking.time.SSP_end_time) {
...
//        }
    ...
    cnt++;
        ...
        thread_t2[3][cnt2]=(long)(t_imu_d - t_imu_s)/1000000.0; // dt_imu_recive
        thread_t2[4][cnt2]=(long)(t_l - t_s)/1000000.0; //dt_task_thread_time
        thread_t2[5][cnt2]=(thread_t); //dt_thread
         ...
         if(cnt2==800)
        {
             ...
             thread_is_running=false;
             save_file();
            DXL_Controller_.Torque_OFF_dxls();
        }
        cnt2++;       
    }
...
void init_pose() {
    ...
    //coswave를 넣어준 이유 아마 z좌표는 preview control을 해주지 않기 때문에 연속적인 궤적이 나오지 않는다 
    //따라서 coswave를 통해 궤적을 만들어 준다.
...
void process() {
              ...
              if (C.walking.time.sec >= C.walking.time.periodTime) {//이거 뭐지
                  C.walking.step.current++;
